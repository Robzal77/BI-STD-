...
This script runs governance checks on all PBIP files in the Batch Test PBI folder,
...
BATCH_TEST_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'Batch Test PBI')
...
def scan_batch_folder():
    """Scan Batch Test PBI folder for all PBIP files"""
...
def print_summary_report(batch_id, results):
    ...
    if failed:
        ...
        for report in failed:
            print_colored(f"  [FAIL] {report['report_name']}", Colors.RED + Colors.BOLD)
...
def run_batch_test():
    ...
    if not reports:
        print_colored("\n[!] No PBIP files found in Batch Test PBI folder", Colors.YELLOW)
        print_colored("\nTo use batch testing:", Colors.GREY)
        print_colored("  1. Copy .pbip folders to 'Batch Test PBI' folder", Colors.GREY)
...
def run_governance_check(report):
    ...
    try:
        ...
        if auto_dt_enabled:
            result['auto_datetime'] = 'FAIL'
            result['status'] = 'FAIL'
            result['failure_reasons'].append('Auto Date/Time is ENABLED')
        ...
        if bidir_rels:
            result['status'] = 'FAIL'
            result['failure_reasons'].append(f'{len(bidir_rels)} Bidirectional relationship(s) found')
        ...
        if missing_descs:
            result['status'] = 'FAIL'
            result['failure_reasons'].append(f'{len(missing_descs)} Measure(s) missing descriptions')
...
def run_batch_test():
    ...
    if not reports:
        ...
        print_colored("\nTo use batch testing:", Colors.GREY)
        print_colored("  1. Copy .pbip folders to 'Batch Test PBI' folder", Colors.GREY)
        print_colored("  2. Run this script again\n", Colors.GREY)
        return
